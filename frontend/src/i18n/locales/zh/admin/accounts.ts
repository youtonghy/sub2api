export default {
    accounts: {
      ...
      modelTest: {
        ...
        summary: '{accounts} 个账号，{models} 个模型测试项',
        hint: '点击模型可单独测试；绿色表示成功，红色表示失败。',
        verificationHint: '选择一个 GPT-5.6 模型，再选择单个账号或全部账号。每个模型都会保留上次报告。', previousReports: '历史报告', modelsReported: '个模型已有报告',
        testAll: '批量测试全部',
        testingProgress: '测试中 {done}/{total}',
        loadingModels: '正在加载账号模型...',
        noAccounts: '当前筛选条件下没有账号',
        ...
        verificationLevel: '验证强度', levelLow: '快速（19次请求）', levelMedium: '标准（49次请求）', levelHigh: '严格（158次请求）', verifyAuthenticity: '验证真实性', verifying: '验证中...', verificationReport: '模型真实性报告', accountsChecked: '个账号', probesMatched: '项有效', viewEvidence: '查看证据', verificationFailed: '真实性验证失败', claimMismatch: '与申报模型不一致',
        verdicts: {
          juice_pass_fingerprint_strong: 'Juice 通过；行为指纹强烈匹配',
          juice_pass_fingerprint_unclear: 'Juice 通过；行为指纹证据不明确',
          juice_mismatch_fingerprint_strong: '确定性检查不一致；行为指纹强烈匹配',
          juice_mismatch_fingerprint_unclear: '确定性检查不一致；行为指纹证据不明确',
          juice_insufficient_fingerprint_strong: 'Juice 证据不足；行为指纹强烈匹配',
          juice_insufficient_fingerprint_unclear: 'Juice 与行为指纹证据均不足',
          possible_non_gpt: '可能非 GPT',
          strong_match: '行为指纹强烈匹配',
          evidence_unclear: '行为指纹证据不明确',
        },
      },
      failureHeatmap: {
        title: '今日账号故障率',
